...
In our procedural provider we want all cameras to look to north, so we use the `[Math.PI / 2, 0, 0]` angle-axis rotation for the camera to world reference frame transform.
