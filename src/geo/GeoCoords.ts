...
export class GeoCoords {
    ...
    private _wgs84b: number = 6356752.314245;
    ...
    public topocentric_from_lla(lat: number, lon: number, alt: number, reflat: number, reflon: number, refalt: number): number[] {
        ...
        let TM: THREE.Matrix4 = new THREE.Matrix4().getInverse(this._ecef_from_topocentric_transform(reflat, reflon, refalt));
        let T: Float32Array = TM.elements;
        let p: number[] = this._ecef_from_lla(lat, lon, alt);
        let x: number = p[0];
        let y: number = p[1];
        let z: number = p[2];
    ...
    public lla_from_topocentric(x: number, y: number, z: number, reflat: number, reflon: number, refalt: number): number[] {
        ...
        let T: Float32Array = this._ecef_from_topocentric_transform(reflat, reflon, refalt).elements;
        let ex: number = T[0] * x + T[4] * y + T[8] * z + T[12];
        let ey: number = T[1] * x + T[5] * y + T[9] * z + T[13];
        let ez: number = T[2] * x + T[6] * y + T[10] * z + T[14];

        return this._lla_from_ecef(ex, ey, ez);
    }

    private _ecef_from_lla(lat: number, lon: number, alt: number): number[] {
        // compute ECEF XYZ from latitude, longitude and altitude.
        // all using the WGS94 model.
        // altitude is the distance to the WGS94 ellipsoid.
        // check results here http://www.oc.nps.edu/oc2902w/coord/llhxyz.htm

        let a2: number = this._wgs84a * this._wgs84a;
        let b2: number = this._wgs84b * this._wgs84b;
    ...
    private _lla_from_ecef(x: number, y: number, z: number): number[] {
    ...
    private _ecef_from_topocentric_transform(lat: number, lon: number, alt: number): THREE.Matrix4 {
        ...
        let eps: number = 1e-6;
        let p: number[] = this._ecef_from_lla(lat, lon, alt);
        let px: number[] = this._ecef_from_lla(lat, lon + eps, alt);
        let mx: number[] = this._ecef_from_lla(lat, lon - eps, alt);
        let py: number[] = this._ecef_from_lla(lat + eps, lon, alt);
        let my: number[] = this._ecef_from_lla(lat - eps, lon, alt);
        let pz: number[] = this._ecef_from_lla(lat, lon, alt + eps);
        let mz: number[] = this._ecef_from_lla(lat, lon, alt - eps);
