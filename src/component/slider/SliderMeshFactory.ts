...
import { SliderShaders } from "./SliderShaders";
...
export class SliderMeshFactory {
    ...
    public createMesh(image: Image, transform: SliderTransform): THREE.Mesh {
    ...
    public createCurtainMesh(image: Image, transform: SliderTransform): THREE.Mesh {
        if (isSpherical(transform.cameraType)) {
    ...
    public createDistortedCurtainMesh(image: Image, transform: SliderTransform): THREE.Mesh {
        return this._createDistortedCurtainMesh(image, transform);
    }

    private _createCurtainMesh(image: Image, transform: SliderTransform): THREE.Mesh {
        let texture: THREE.Texture = this._createTexture(image.image);
        let materialParameters: THREE.ShaderMaterialParameters =
    ...
    private _createCurtainMeshFisheye(image: Image, transform: SliderTransform): THREE.Mesh {
        let texture: THREE.Texture = this._createTexture(image.image);
        let materialParameters: THREE.ShaderMaterialParameters =
    ...
    private _createDistortedCurtainMesh(image: Image, transform: SliderTransform): THREE.Mesh {
        let texture: THREE.Texture = this._createTexture(image.image);
        let materialParameters: THREE.ShaderMaterialParameters =
    ...
    private _createSphereCurtainMesh(image: Image, transform: SliderTransform): THREE.Mesh {
        let texture: THREE.Texture = this._createTexture(image.image);
        let materialParameters: THREE.ShaderMaterialParameters =
    ...
    private _createImageSphere(image: Image, transform: SliderTransform): THREE.Mesh {
        let texture: THREE.Texture = this._createTexture(image.image);
    ...
    private _createImagePlane(image: Image, transform: SliderTransform): THREE.Mesh {
        let texture: THREE.Texture = this._createTexture(image.image);
    ...
    private _createImagePlaneFisheye(image: Image, transform: SliderTransform): THREE.Mesh {
        let texture: THREE.Texture = this._createTexture(image.image);
    ...
    private _createSphereMaterialParameters(transform: SliderTransform, texture: THREE.Texture): THREE.ShaderMaterialParameters {
        let materialParameters: THREE.ShaderMaterialParameters = {
            depthWrite: false,
            fragmentShader: SliderShaders.spherical.fragment,
            ...
            vertexShader: SliderShaders.spherical.vertex,
        };

        return materialParameters;
    }

    private _createCurtainSphereMaterialParameters(transform: SliderTransform, texture: THREE.Texture): THREE.ShaderMaterialParameters {
        let materialParameters: THREE.ShaderMaterialParameters = {
            depthWrite: false,
            fragmentShader: SliderShaders.sphericalCurtain.fragment,
            side: THREE.DoubleSide,
            transparent: true,
            uniforms: {
                curtain: { value: 1.0 },
            ...
            vertexShader: SliderShaders.sphericalCurtain.vertex,
        };

        return materialParameters;
    }

    private _createPlaneMaterialParameters(
        transform: SliderTransform,
        texture: THREE.Texture)
        : THREE.ShaderMaterialParameters {
        let materialParameters: THREE.ShaderMaterialParameters = {
            depthWrite: false,
            fragmentShader: SliderShaders.perspective.fragment,
            side: THREE.DoubleSide,
            transparent: true,
            uniforms: {
            ...
            vertexShader: SliderShaders.perspective.vertex,
        };

        return materialParameters;
    }

    private _createPlaneMaterialParametersFisheye(
        transform: SliderTransform,
        texture: THREE.Texture)
        : THREE.ShaderMaterialParameters {
        let materialParameters: THREE.ShaderMaterialParameters = {
            depthWrite: false,
            fragmentShader: SliderShaders.fisheye.fragment,
            side: THREE.DoubleSide,
            transparent: true,
            uniforms: {
                focal: { value: transform.focal },
            ...
            vertexShader: SliderShaders.fisheye.vertex,
        };

        return materialParameters;
    }

    private _createCurtainPlaneMaterialParametersFisheye(
        transform: SliderTransform,
        texture: THREE.Texture)
        : THREE.ShaderMaterialParameters {
        let materialParameters: THREE.ShaderMaterialParameters = {
            depthWrite: false,
            fragmentShader: SliderShaders.fisheyeCurtain.fragment,
            side: THREE.DoubleSide,
            transparent: true,
            uniforms: {
                curtain: { value: 1.0 },
            ...
            vertexShader: SliderShaders.fisheyeCurtain.vertex,
        };

        return materialParameters;
    }

    private _createCurtainPlaneMaterialParameters(
        transform: SliderTransform,
        texture: THREE.Texture)
        : THREE.ShaderMaterialParameters {
        let materialParameters: THREE.ShaderMaterialParameters = {
            depthWrite: false,
            fragmentShader: SliderShaders.perspectiveCurtain.fragment,
            side: THREE.DoubleSide,
            transparent: true,
            uniforms: {
                curtain: { value: 1.0 },
            ...
            vertexShader: SliderShaders.perspectiveCurtain.vertex,
        };

        return materialParameters;
    }

    private _createDistortedCurtainPlaneMaterialParameters(
        transform: SliderTransform,
        texture: THREE.Texture)
        : THREE.ShaderMaterialParameters {
        let materialParameters: THREE.ShaderMaterialParameters = {
            depthWrite: false,
            fragmentShader: SliderShaders.perspectiveDistortedCurtain.fragment,
            side: THREE.DoubleSide,
            transparent: true,
            uniforms: {
                curtain: { value: 1.0 },
            ...
            vertexShader: SliderShaders.perspectiveDistortedCurtain.vertex,
        };

        return materialParameters;
    }

    private _createDistortedPlaneMaterialParameters(
        transform: SliderTransform,
        texture: THREE.Texture)
        : THREE.ShaderMaterialParameters {
        let materialParameters: THREE.ShaderMaterialParameters = {
            depthWrite: false,
            fragmentShader: SliderShaders.perspectiveDistorted.fragment,
            side: THREE.DoubleSide,
            transparent: true,
            uniforms: {
                opacity: { value: 1.0 },
            ...
            vertexShader: SliderShaders.perspectiveDistorted.vertex,
        };
    ...
    private _getImagePlaneGeo(transform: SliderTransform, image: Image): THREE.BufferGeometry {
    ...
    private _getImagePlaneGeoFisheye(transform: SliderTransform, image: Image): THREE.BufferGeometry {
        const t = transform.srtInverse;
    ...
    private _getFlatImageSphereGeo(transform: SliderTransform): THREE.BufferGeometry {
        const geometry =
    ...
    private _getRegularFlatImagePlaneGeo(transform: SliderTransform): THREE.BufferGeometry {
    ...
    private _getFlatImagePlaneGeo(transform: SliderTransform, dx: number, dy: number): THREE.BufferGeometry {
        let vertices: number[][] = [];
    ...
    private _getRegularFlatImagePlaneGeoFisheye(transform: SliderTransform): THREE.BufferGeometry {
        let width: number = transform.width;
        let height: number = transform.height;
        let size: number = Math.max(width, height);
        let dx: number = width / 2.0 / size;
    ...
    private _getFlatImagePlaneGeoFisheye(transform: SliderTransform, dx: number, dy: number): THREE.BufferGeometry {
        let vertices: number[][] = [];
        vertices.push(transform.unprojectSfM([-dx, -dy], this._imagePlaneDepth));
        vertices.push(transform.unprojectSfM([dx, -dy], this._imagePlaneDepth));
        vertices.push(transform.unprojectSfM([dx, dy], this._imagePlaneDepth));
